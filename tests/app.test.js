...
// Espera a que las promesas pendientes resuelvan
...
describe('sendToGemini — caso exitoso', () => {
  it('llama a /api/functions con método POST', async () => {
    fetch.mockResolvedValueOnce({
      ok: true,
      json: async () => ({ reply: '¡Seré el Rey de los Piratas!' }),
    });
  ...
  it('agrega la respuesta de Luffy al chat', async () => {
    fetch.mockResolvedValueOnce({
      ok: true,
      json: async () => ({ reply: '¡Seré el Rey de los Piratas!' }),
    });
    ...
    expect(container.textContent).toContain('¡Seré el Rey de los Piratas!');
  });
});
